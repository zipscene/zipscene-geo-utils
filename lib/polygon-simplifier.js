...
class PolygonSimplifier {
	...
	/**
	 * Calls simplify until the polygon reaches a state defined by provided options.
	 *
	 * @method simplifyTo
	 * @param {Object} options - A set of named options describing the target
	 *   state of the polygon. If left empty, this method will do nothing.
	...
	simplifyTo({ maxVertices, minVertices, maxError }) {
		...
		// Function for checking if simplification should continue based on
		// maxVertices.
		let exceedsMaxVertices = () => hasMaxVertices &&
			this.vertexCount > maxVertices;

		// Function for checking if simplification should continue based on
		// minVertices and/or maxError.
